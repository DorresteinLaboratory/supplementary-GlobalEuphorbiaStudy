...
sc <- read.table("gnps_node_attributes_3DMolecularCartography.tsv",sep="\t",header = T,comment.char = "",stringsAsFactors = F,quote="")  # downloaded from https://gnps.ucsd.edu/ProteoSAFe/status.jsp?task=cbd1ea3bc17045188d2184c4289f3023 (Download Cytoscape data, clusterinfosummarygroup_attributes_withIDs_withcomponentID)

ci <- unique(sc$componentindex)
ci <- ci[-which(ci==-1)] # remove single nodes

# downloaded from https://proteomics2.ucsd.edu/ProteoSAFe/status.jsp?task=6d58b49a645d408e9471f82d1b308f6a
naph <- read.table("node_attributes_table_H.tsv",sep="\t",quote = "",header=T)
# downloaded from https://proteomics2.ucsd.edu/ProteoSAFe/status.jsp?task=425dd24f42d54b4ba58aba69f2f1dfac 
...
for (i in 1:length(ci)){
  ...
  NAPNH4 <- napnh4[which(napnh4$cluster.index %in% sc_smiles$cluster.index),]
  ...
  allsmiles <- unique(c(HGNPS,HMetFrag,HFusion,HConsensus,NH4GNPS,NH4MetFrag,NH4Fusion,NH4Consensus))
...
write.table(smnet,"ClassyFire_InputSMILES_3DMolecularCartography.tsv",sep="\t",row.names = F,quote = F,col.names = F)

# ClassyFire_InputSMILES_3DMolecularCartography.tsv was submited to ClassyFire (http://classyfire.wishartlab.com/) [Djoumbou Feunang et al., 2016] and .json files were downloaded
...
part1 <- "http://classyfire.wishartlab.com/queries/2813075.json"

files_p1 <- paste(part1, "?page=", 1:46,sep="")

files <- files_p1

jsonl <- lapply(files, function(f) tryCatch({fromJSON(f)},error=identity))
files[which(vapply(jsonltest, is, logical(1), "error")==T)]
...
write.table(finalout,"ClassyFire_Output_forCytoscape_3DMolecularCartography.tsv",sep="\t",row.names = F,quote = F)
